...
const Category = (props) => {
    ...
    if (props.children.length) {
        childCategories = props.children.map((childCategory) => {
            return (
                <Category
                    ...
                    handleSpecifyCategoryToMove={props.handleSpecifyCategoryToMove}
                    transferTaskIn={props.categoryToMoveTaskInId === childCategory.id}
                    categoryToMoveTaskInId={props.categoryToMoveTaskInId}
                />
            )
        })
    }
    ...
    switch (props.mode) {
        case 'taskTransfer':
            return (
                <li className="categories__item" key={props.node.id}>
                    <span className="categories__text">{props.node.name}</span>
                    <div className="categories__manage-list">
                        <button
                            className={
                                props.transferTaskIn ? "button  button--manage  button--transfer-approved" : "button  button--manage  button--transfer"
                            }
                            onClick={() => {
                                props.handleSpecifyCategoryToMove(props.node.id)
                            }}
                            disabled={props.categoryID === props.node.id}
                        >
		...
		C551.587,158.028,520.541,411.234,241.914,403.974z"/>
                    ...
                    {childCategories ? <ul className="categories__list  categories__list--nested">{childCategories}</ul> : null}
                </li>
            );
        case 'homePage':
            return (
                <li className="categories__item" key={props.node.id}>
                    <NavLink to={`/category/${props.node.id}`}
                             activeClassName="categories__link--active"
                             exact={true}
                             className="categories__link"
                             onClick={() => {props.updateProgressBar(props.node.id)}}
                    >
                        {props.node.name}
                    </NavLink>
                    ...
                    <div className="categories__manage-list">
                        <button
                            ...
                            onClick={() => {
                                props.handleEditCategory(props.node.id)
                            }}
                        >
		...
		L27.473,390.597L0.3,512.69z"/>
                        ...
                        <button
                            className="button  button--manage  categories__manage-item"
                            onClick={() => {
                                props.handleDeleteCategory(props.node.id)
                            }}
                        >
			...
			c-7.125,0-12.9,5.776-12.9,12.901V74.47h304.451V33.944C356.467,26.819,350.692,21.043,343.567,21.043z"/>
                        ...
                        <button
                            ...
                            onClick={() => {
                                props.handleAddNestedCategory(props.node.id)
                            }}
                        >
